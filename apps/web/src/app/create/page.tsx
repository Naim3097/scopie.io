...
/**
 * Creator upload. Real mode: caption + file → direct-to-Cloudflare upload
 * with progress → "processing" (ready-webhook + moderation put it in the
 * feed). Demo mode: instant publish with a stock clip. File validation runs
 * BEFORE any server call — an errant click must not mint upload reservations.
 */
export default function CreatePage() {
  ...
  const post = async () => {
    ...
    if (session.authEnabled && !session.userId) {
      router.push(`/auth?next=${encodeURIComponent(pathname ?? "/create")}`);
      return;
    }
    setError(null);
  ...
  if (stage === "posted" || stage === "processing") {
    ...
    return (
      <main className="page page--pad" style={{ textAlign: "center", paddingTop: 80 }}>
        ...
        <button className="btn btn-primary" style={{ width: "auto" }} onClick={() => router.push("/feed")}>
          Go to feed
        </button>
...
function MyUploads({ uploads }: { uploads: MyUpload[] }) {
  ...
  return (
    <div style={{ marginTop: 26, maxWidth: 440, textAlign: "left" }}>
      <h2 style={{ fontSize: 16, marginBottom: 10 }}>My uploads</h2>
